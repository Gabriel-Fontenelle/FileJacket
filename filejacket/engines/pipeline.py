...
__all__ = ["Processor", "PipelineEngine"]
...
class Processor:
    ...
    @classmethod
    def instantiate(cls, processor: object, parameters: dict[str, Any]) -> object:
        """
    ...
    def validate(cls, processor: object):
        ...
        if not hasattr(processor, "dependencies"):
            raise ValidationError(
                f"Class {processor.__class__.__name__} should implement the attribute `dependencies` to be a "
                f"valid processor class."
            )
